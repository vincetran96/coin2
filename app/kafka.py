...
class KafkaAccSender():
    ...
    """
    def __init__(self, topic: str, batchsize: int, send_timeout: int, key: str = None):
        """
        Args:
    ...
    def send(self):
        if self.data_list:
            if len(self.data_list) >= self.batchsize or time.monotonic() - self.start_time >= self.send_timeout:
                send_to_kafka(producer=self._producer, topic=self.topic, data_list=self.data_list, key=self.key)
